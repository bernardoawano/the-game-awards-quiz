...
- **`eslint-config-next@16.3.0` não suporta ESLint 10** (verificado na Fase 9) — seus plugins internos (`eslint-plugin-react`, `eslint-plugin-jsx-a11y`, `eslint-plugin-import`, todos vendorizados dentro de `node_modules/eslint-config-next/node_modules/`) declaram peer range até `^9`, e travam com `TypeError: contextOrFilename.getFilename is not a function` sob ESLint 10 (a API `context.getFilename()` foi removida nessa major). Não era o conflito de identidade de plugin que eu esperava (`typescript-eslint` já dedupava certo). Corrigido baixando `eslint`/`@eslint/js` da raiz de `^10.8.0` pra `^9.39.5` — confirmado antes via `npm view` que `typescript-eslint@8.65.0` e `eslint-plugin-import-x@4.17.1` (as duas outras dependências de lint do monorepo) já suportam `^9.0.0` nativamente, então não há regressão pro backend.
- **`createTypeScriptImportResolver()` sem a opção `project` só resolve o alias `@/*` de um workspace no monorepo** (verificado na Fase 9) — sem `project` explícito, o resolver não sabia que `frontend/tsconfig.json` também existe, e todo import `@/...` dentro de `frontend/**` dava `import-x/no-unresolved`. Corrigido passando `project: ['backend/tsconfig.json', 'frontend/tsconfig.json']` (com `noWarnOnMultipleProjects: true`, forma documentada pelo próprio pacote pra monorepos) no `settings['import-x/resolver-next']` do bloco TS genérico do `eslint.config.mjs`.
- **`typedRoutes: true` também tipa `redirect()` de `next/navigation`, não só `<Link href>`** (verificado na Fase 9) — `lib/session.ts::requireUser()` chama `redirect('/login')`, mas `/login` só nasce na Fase 11; sem rota real, o `Route` union gerado não inclui a string e `tsc --noEmit` falha. Corrigido com `redirect('/login' as Route)` (o mesmo escape hatch documentado pra strings não-literais), a trocar pelo literal sem cast assim que a Fase 11 criar `/login` de verdade.
- **`lib/api.ts` não pode exportar `serverFetch` e `clientFetch` do mesmo arquivo** (verificado na Fase 9, apesar do `CLAUDE.md` descrever os dois num único módulo) — qualquer arquivo que importe `next/headers` no topo (necessário pro `serverFetch` ler cookies) quebra em runtime (`Ecmascript file had an error`) se um Client Component importar **qualquer coisa** desse mesmo arquivo, mesmo uma função que não usa `cookies()`. O Next analisa o grafo de import inteiro do módulo, não a função específica usada. Resolvido dividindo em três arquivos: `lib/api-error.ts` (`ApiError`, `getApiUrl`, `parseResponse` — sem import do Next, seguro pros dois lados), `lib/api.ts` (importa `next/headers`, exporta `serverFetch` + re-exporta `ApiError`, continua sendo o módulo que Server Components importam) e `lib/api.client.ts` (exporta `clientFetch` + re-exporta `ApiError`, é o que Client Components como `LogoutButton.tsx` importam). "Único ponto de acesso à API" continua valendo no sentido que importa — nenhuma URL hardcoded, um só parser de erro — só não cabe fisicamente num arquivo só.
- **`next dev` gera `frontend/AGENTS.md` e `frontend/CLAUDE.md` automaticamente** (Next.js 16, verificado na Fase 9) — arquivo próprio do framework (regenerado a cada `next dev`, ver `node_modules/next/dist/server/lib/generate-agent-files.js`) avisando agentes de IA que a versão instalada pode divergir do conhecimento de treinamento e apontando pra `node_modules/next/dist/docs/`. Não conflita com o `CLAUDE.md` da raiz deste projeto (arquivo e propósito diferentes) — mantido e commitado, como o próprio Next recomenda (`agentRules: false` em `next.config.ts` desativaria a geração, mas não há motivo pra isso aqui).
